...
satang.show_info()
'''
속성값에 대한 제한이 따로 있지 않다면(예를 들어 나이에 123098살을 입력하는 등의)
굳이 빈 객체 만들어놓고 거기에 값 대입하는 것이 비효율적으로 느껴집니다. 
그래서 저희는 AllArgsConstructor를 도입할겁니다.

Java / JS에서의 생성자 명은 클래스 명과 동일하거나 constructor를 쓰는데,
python만 또 지 혼자서 이상한거 씁니다. __init__() 입니다. 언더스코어 두 개임.
'''
class Candy2:
    # 생성자 정의
...
'''
지금 보니까 코드 다 돌아가면 객체가 소멸되는 것 같은데 굳이 소멸자를 학습하는 이유 -> 
객체가 생성되면 메모리 공간을 차지하기 때문에 객체가 생성될 때마다 그곳에서 불려나오게 됩니다. 
하지만 특정 객체가 일정 코드라인 이후로 전혀 사용되지 않을 때에도 
여전히 메모리를 차지하기 때문에 소멸자를 통해서 강제로 객체를 삭제해주게 되면 메모리 관리가 된다고 볼 수 있습니다.
